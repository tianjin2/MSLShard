...
func txGenerator(flagArgs *FlagArgs, allNodes []NodeAllInfo, users *[]PrivKey, gensisBlocks []*FinalBlock, finalBlockChan chan FinalBlock, files []*os.File,isSend chan bool,
	committees map[[32]byte]*Committee, nodeT map[[32]byte]sendToNodeTrust) {
	...
	for {
		...
		for i := 0; i < l; i++ {
			...
			for _, t := range finalBlock.ProposedBlock.Transactions {
				...
				delete(transactionTracker.m, id)
				transactionTracker.mux.Unlock()
		...
		ends := time.Now()
		times += ends.Sub(now).Seconds()
		after := time.Now()

		if *isSendTx {
			*count++
			go _txGenerator(flagArgs, &allNodes, users, userSets, transactionTracker,ls,*count,isSendTx)
		}
		if len(transactionTracker.m) == 0 && *count !=1 {
			reconfiguration_ra(committees,&allNodes,int(flagArgs.n),int(flagArgs.m),nodeT)
			for {
		...
		dur1 := (time.Second / time.Duration(flagArgs.tps)) - after.Sub(before)
...
func _txGenerator(flagArgs *FlagArgs, allNodes *[]NodeAllInfo, users *[]PrivKey, userSets *UserSets, transactionTracker *TransactionTracker,l *int ,count int,isSendTx *bool) {
