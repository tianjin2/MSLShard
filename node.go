...
func nodeHandleConnection(
	...
	case "NodeTrust":
		dMsg, ok := msg.Msg.(sendCoordToNodeLocalTrust)
		notOkErr(ok,"decision decoding")
		nodeCtx.nodeTrust.Af = dMsg.Af
		nodeCtx.nodeTrust.ETrust = dMsg.ETrust
		nodeCtx.nodeTrust.nodeTrust = dMsg.Trust
		nodeCtx.nodeTrust.Trust = dMsg.Trust[nodeCtx.self.Priv.Pub.Bytes].trust
		if nodeCtx.i.flag[nodeCtx.i.i] {
			nodeCtx.i.flag[nodeCtx.i.i] = false
		}
	case "decision":
		dMsg, ok := msg.Msg.(DecisionVector)
		notOkErr(ok,"decision decoding")
		go receiveDec_ra(nodeCtx,dMsg,msg.FromPub.Bytes)
	case "find_node":
